...
const createProdSafePrismaClient = () => {
  // Create a normal PrismaClient with connection retry and pooling
  const client = new PrismaClient({
    log: process.env.NODE_ENV === 'development' ? ['query', 'error', 'warn'] : ['error'],
    // Add connection pooling for more stability
    datasources: {
      db: {
  ...
  // Warmup connection to avoid cold starts
  (async () => {
    try {
      // Connect to the database to verify connection works
      await client.$connect();
      console.log('Database connection established successfully');
    } catch (err) {
      console.error('Failed to establish database connection on startup:', err);
    }
  })();
      ...
      if (typeof original === 'object') {
        return new Proxy(original, {
          get: (modelTarget: any, methodProp: string) => {
            ...
            // Wrap the method with error handling and retry logic
              ...
              while (true) {
                try {
                  ...
                  if (retries >= maxRetries || 
                    ...
                    console.error(`Prisma error in ${String(prop)}.${String(methodProp)} after ${retries} retries:`, error);
                  ...
                  retries++;
                  console.warn(`Retrying database operation after error (retry ${retries}/${maxRetries}): ${error.message}`);
...
// Export a wrapper for the Prisma db instance for use in webhook handlers
